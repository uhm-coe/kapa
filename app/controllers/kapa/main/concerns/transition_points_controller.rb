module Kapa::Main::Concerns::TransitionPointsController
  ...
  def index
    @filter = filter
    @transition_points = TransitionPoint.search(@filter).order("persons.last_name, persons.first_name").paginate(:page => params[:page], :per_page => @filter.per_page)
  end
